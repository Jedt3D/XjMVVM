...
XjMVVM v0.9.3 では 2 つの関連機能が導入されました：認証が必要な**保護されたルート**と、レコードをユーザーごとに分離する**ユーザースコープデータ**です。これら 2 つを組み合わせることで、ユーザーはアプリにアクセスするためにログインする必要があり、自分のノートのみを表示できます。

## 保護されたルート

認証が必要な ViewModel は、すべて `OnGet()` または `OnPost()` の最上部でガード メソッドを呼び出します。2 つのガード があります—1 つは HTML ルート用、もう 1 つは API ルート用です。
...
`RequireLogin()` は `ParseAuthCookie()` を呼び出して HMAC 署名された `mvvm_auth` クッキーを検証します。無効または欠落している場合は、`/login?next=<encoded-current-path>` にリダイレクトされるため、ログイン後、ユーザーは元のアドレスに戻ります。
...
API クライアント（JavaScript `fetch`、モバイル アプリ、CLI ツール）は HTML リダイレクトに従うことができません。`RequireLoginJSON()` は代わりに 401 ステータスと JSON エラー本文を返します：
...
Auth ルート（`/login`、`/signup`、`/logout`）は**保護されていません**—ユーザーはログインせずにこれらに到達できる必要があります。

### ログイン後の `next` パラメータによるリダイレクト

`RequireLogin()` が `/login` にリダイレクトするとき、現在の URL をクエリ パラメータとして追加します：

```
/login?next=%2Fnotes%2F42%2Fedit
```

`LoginVM` はこの `next` パラメータを読み取り、ログイン成功後に `RedirectWithAuth()` に渡し、ユーザーを戻ろうとしていた場所に送ります：
...
ノートはユーザーごとにスコープされます—各ユーザーは独自のノート セットを持ちます。タグは引き続きグローバル（すべてのユーザー間で共有）ですが、アクセスするには引き続きログインが必要です。
...
`DEFAULT 0` は、既存のノート（認証が存在する前に作成）がアクセス可能なままであることを保証します。新しいノートには認証されたユーザーの ID が付与されます。
...
`GetByID`、`Update`、および `Delete` はすべて WHERE 句に `AND user_id = ?` を含みます。ユーザー A がユーザー B のノートに ID を推測してアクセスしようとする場合、クエリは `Nil` を返す（または 0 行に影響する）、ViewModel は 404 を返します：
...
別の「あなたはこれを所有していない」エラーはありません—ノートは現在のユーザーの視点から単に存在しません。

### タグ — グローバルだが保護されている

タグはすべてのユーザー間で共有されます。ログインが必要ですが、ユーザーによってスコープされていません：
...
この設計は、すべてのユーザーが同じ タグ の語彙を共有することを意味します。1 人のユーザーが作成したタグはすべてのユーザーが利用できます。

---

## 所有権のテスト

`NoteOwnershipTests` はユーザー スコーピングが正しく機能することを検証します：
...
これらのテストは異なる `userID` 値（999 対 888）を使用して、SQL レベルのスコーピングがユーザー間のアクセスを防ぐことを証明します。