...
Tagsを追加することにより、同時に2つのことが導入されました：**2番目の完全なリソース**（Notes以上にCRUDパターンが一般化されることを証明する）と、結合テーブルを経由したNotesとTagsの**多対多リレーションシップ**です。
...
複合主キー`(note_id, tag_id)`はデータベースレベルで一意性を強制します — noteは同じtagと2回関連付けられることはできません。
...
`NoteModel.SetTagsForNote()`は**削除してから挿入する**パターンを使用してタグセット全体をアトミックに置き換えます：
...
!!! note
    削除してから挿入するパターンはタグセット全体を扱います — これは古いセットと新しいセットをディフェレンシエートしません。これはシンプルで、ほとんどのユースケースに正しいです。各関連付けのメタデータを保持する必要がある場合（例：タグ割り当てごとのタイムスタンプ）は、ディフェレンシエートアプローチが必要です。
...
**2.** `GetCategoriesForNote()`と`SetCategoriesForNote()`を、同じJOINおよび削除-挿入パターンを使用して関連するモデルに追加してください。

**3.** noteの詳細またはフォーム編集をレンダリングするViewModelで、両方のメソッドを呼び出し、結果をコンテキストディクショナリにマージしてください。
