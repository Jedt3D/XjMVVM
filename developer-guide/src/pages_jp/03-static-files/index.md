...
`HandleURL` がすべてのリクエストをインターセプトするため、静的アセット (`.css`、`.js`、画像) のリクエストを明示的に処理する必要があります。最も単純から最も柔軟な順序でソートされた 3 つのアプローチがあります。
...
変更しないライブラリにこれを使用します。デプロイメント複雑さはゼロです。

## オプション 2 — 静的ファイル ViewModel

独自の CSS、JS、画像の場合、ディスクからファイルを読み取り、レスポンスに書き込む `/static/:file` ルートを登録します。
...
`App.Opening()` で、すべての他のルート **後に** 静的ルートを登録して、アプリケーションパスと誤ってマッチするのを避けます:
...
!!! warning "セキュリティ"
    ディスクから読み取る前に常にファイル名を検証します。上の例の path traversal チェック (`..`, `/`, `\`) は最小限の必須です。ユーザー入力なしでファイルパスを構築しないでください。
...
このアプローチは、アプリケーションで出荷され、実行時に決して変わらないファイルに最適です。開発中に頻繁に編集する開発者アセットの場合、オプション 2 がより便利です。
