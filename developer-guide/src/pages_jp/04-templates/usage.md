...
double curly braces で変数を出力します。HTML は自動的にエスケープされます — `<script>` は `&lt;script&gt;` になります:
...
Jinja2 のフォルシー値: `False`、`0`、`""` (空文字列)、`[]` (空配列)、`None`/`Nil`。存在しないキーはフォルシーとして扱われます。
...
JSON または CSV をテンプレートから生成する場合に有用です。
...
フラッシュメッセージは `BaseViewModel.Render()` によって自動的に注入されます — それらを手動でコンテキストに追加しません。
