---
title: なぜ XjMVVM?
description: XjMVVM の背後にある建築的な決定 — なぜ Xojo の WebPage システムを回避し、なぜ JinjaX を使用した MVVM パターンなのか。
---

# なぜ XjMVVM?

このページはフレームワークデザイン背後の主要な決定を説明します。*なぜ*を理解することで、このガイド内の他のすべての規約が意味をなします。
...
**デザイナーフレンドリーなテンプレート。** HTML テンプレートは `templates/` フォルダに存在します。デザイナーまたはフロントエンド開発者は、完全な IDE サポート (構文ハイライト、Emmet、Prettier) を備えたテキストエディタを使用して編集できます。彼らは Xojo コードに触れることはありません。

**UI レイヤーにセッション状態がない。** Xojo WebControl は暗黙的なセッション状態を持ちます。私たちの ViewModel はリクエストごとにステートレスです — リクエストを受け取り、作業し、レスポンスを書きます。唯一の永続的なコストは `Session` (`WebSession` クラス) とデータベースです。
...
JinjaX は Python の Jinja2 テンプレートエンジンを純粋な Xojo で実装したものです。選択された理由は:

**Jinja2 はデファクトスタンダードです。** Flask、Django、Ansible、または Saltstack に慣れた開発者は既に構文を知っています。学習曲線は最小限です。

**それは純粋な Xojo ソースです。** 完全な JinjaX ソースツリーは `JinjaXLib/` の下に存在します。バイナリ `.xojo_library` 依存性はないため、Xojo の text プロジェクト形式 (`.xojo_code` ファイル) で機能し、完全に検査可能でデバッグ可能です。
...
JinjaX はドット記法 (`{{ note.title }}`) をオブジェクトの `EvaluateGetAttr` を呼び出すことで解決します。Xojo では、`EvaluateGetAttr` は `Dictionary` に対してのみ実装されており、名前でキーを検索します。カスタム Xojo クラスインスタンスは `EvaluateGetAttr` をサポートしないため、ドットアクセスはサイレントに空として レンダリングされます。
...
検証が失敗した場合、ViewModel はリダイレクト **しません** — 同じ GET テンプレートを使用してエラーフラッシュメッセージでフォームを再レンダリングします:
...
ユーザーごとの状態は `Session` クラス (`WebSession` から継承) にあります。各ブラウザセッションは、Xojo のランタイムによって自動的に独自の `Session` インスタンスを取得します:
