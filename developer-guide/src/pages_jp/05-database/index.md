...
戻り型を `Dictionary` オブジェクトを含む `Variant()` にするのは、JinjaX が `{% for %}` ループで反復できる唯一の形式だからです。
...
プライベートの `OpenDB()` メソッドは呼び出しごとに**新しい**接続を開きます。`App` に共有された長期接続は存在しません。
...
**なぜリクエストごとか?** Xojo は複数のスレッドで同時リクエストを処理します。共有 `SQLiteDatabase` インスタンスにはミューテックスが必要です。リクエストごとに新しい接続を開く方がシンプルで、ロック周りの複雑さをすべて回避できます — SQLite は複数プロセスからの同時読み取り接続をネイティブに処理します。

**なぜ接続プールではないか?** このフレームワークが対象とするトラフィック量（小チーム、内部ツール）では、接続を開くオーバーヘッドは無視できます。このスケールではプールは複雑さを加えるだけで、測定可能なメリットはありません。
...
起動のたびに呼び出しても安全です — `IF NOT EXISTS` はべき等です。

## パラメータ化されたクエリ

値には常に `?` プレースホルダーを使用します。ユーザー入力を SQL 文字列に連結しないでください — SQL インジェクションを防ぐためです:
...
SQLite の `datetime()` 関数を使用してすべての日付を `TEXT` として保存します。文字列として取得して表示し、必要に応じてテンプレートまたは ViewModel でフォーマットします。
