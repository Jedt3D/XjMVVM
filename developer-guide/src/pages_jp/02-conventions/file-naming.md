...
- **VM** サフィックスは即座にこれが ViewModel であることを明白にします
...
Xojo はランタイムでクラス名を使用します — ファイル名ではなく — しかし、それらを同じに保つことは混乱を防ぎます。
...
ViewModel はコンテキスト辞書で `note` を渡すか省略することでモードを制御します。

## ルート登録命名

`App.Opening()` でルートを登録するとき、順序と一致させます: POST の前に GET、詳細の前にリスト:
