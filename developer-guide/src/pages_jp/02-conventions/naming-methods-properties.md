...
これらは意図的にイベントハンドラーのように命名されています — それらは HTTP メソッドに **応答します**、何も開始しません。`OnGet()` のみをオーバーライドする場合、そのルートへの `POST` は基本クラスから自動的に 405 レスポンスを取得します。
...
パブリックプロパティ (`BaseViewModel` によって継承または設定されたもの) は接頭辞がありません:
...
モデルメソッドは `Shared` です — モデルクラスをインスタンス化する必要はありません:
...
`Dictionary` オブジェクト内のキー — テンプレートのコンテキスト辞書とモデルの戻り値の両方 — は `snake_case` 文字列を使用します。これは SQLite のカラム名と Jinja2 テンプレートで使用される変数名に一致します:
...
ctx.Value("flash")      = ...                   // Render() で自動的に注入
```
...
`GetFormValue(key)` — URL エンコードされた POST ボディからフィールドを読み取ります。初回呼び出しでボディを遅延的に解析し、結果をキャッシュします。
...
`Render(templateName, context)` — テンプレートをコンパイルしてレンダリング、フラッシュメッセージを自動注入、HTML レスポンスを書き込みます。
...
`SetFlash(message, type)` — セッションにフラッシュメッセージを保存します。任意の ViewModel での次の `Render()` 呼び出しはそれをテンプレートコンテキストに自動的に注入します。タイプは `"success"`、`"error"`、または `"info"`。
