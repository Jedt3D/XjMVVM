...
description: วิธีการปกป้องเส้นทางด้วยการรับรองความถูกต้อง และจำกัดข้อมูลต่อผู้ใช้เพื่อให้ผู้ใช้แต่ละคนเห็นเฉพาะบันทึกของตัวเองเท่านั้น
---

# เส้นทางที่ปกป้อง & การจำกัดขอบเขตข้อมูลต่อผู้ใช้

XjMVVM v0.9.3 นำเสนอฟีเจอร์ที่เกี่ยวข้องกันสองประการ: **เส้นทางที่ปกป้อง** ซึ่งต้องการการรับรองความถูกต้อง และ **ข้อมูลที่จำกัดต่อผู้ใช้** ที่แยกบันทึกต่อผู้ใช้ รวมกันแล้ว ระบบจึงมั่นใจว่าผู้ใช้จะต้องเข้าสู่ระบบเพื่อเข้าถึงแอปพลิเคชันและสามารถเห็นได้เฉพาะบันทึกของตัวเองเท่านั้น

## เส้นทางที่ปกป้อง

ViewModel ที่ต้องการการรับรองความถูกต้องจะเรียกเมธอด guard ที่ด้านบนสุดของ `OnGet()` หรือ `OnPost()` ประเทศไทยมีสองวิธีการยาม — วิธีหนึ่งสำหรับเส้นทาง HTML และอีกวิธีหนึ่งสำหรับเส้นทาง API
...
ไคลเอนต์ API (JavaScript `fetch`, แอปมือถือ, เครื่องมือ CLI) ไม่สามารถติดตามการเปลี่ยนเส้นทาง HTML ได้ `RequireLoginJSON()` จะส่งกลับสถานะ 401 พร้อมบริเวณเนื้อความข้อผิดพลาด JSON แทน:
...
ทั้ง 19 เส้นทาง ViewModel ต้องการการรับรองความถูกต้อง:
...
เส้นทางการรับรองความถูกต้อง (`/login`, `/signup`, `/logout`) **ไม่** ได้รับการปกป้อง — ผู้ใช้จะต้องสามารถเข้าถึงได้โดยไม่ต้องเข้าสู่ระบบ
...
`DEFAULT 0` มั่นใจว่าบันทึกที่มีอยู่ก่อน (สร้างขึ้นก่อนการมีอยู่ของการรับรองความถูกต้อง) ยังคงสามารถเข้าถึงได้ บันทึกใหม่ได้รับ ID ของผู้ใช้ที่ผ่านการรับรองความถูกต้อง
...
ViewModel Notes ทุกอันอ่าน `CurrentUserID()` (จาก Cookie การรับรองความถูกต้อง) และส่งผ่านไปยังโมเดล:
...
1. ผู้ใช้จะต้องผ่านการรับรองความถูกต้อง (`RequireLogin`)
