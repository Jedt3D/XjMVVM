...
Alpine ไม่ใช่เครื่องมือสำหรับแทน business logic ฝั่งเซิร์ฟเวอร์ กฎธุรกิจ ข้อมูล และการตรวจสอบทุกอย่างที่ทำได้บนเซิร์ฟเวอร์ ให้อยู่บนเซิร์ฟเวอร์
...
`@submit` บนฟอร์ม logout ล้างข้อมูลใน localStorage และเตรียม flash message ก่อนที่ POST จะถูกส่ง โดยไม่ต้องเพิ่ม event listener หรือ script block แยกต่างหาก

---

## รูปแบบ 2 — ข้อความแฟลชฝั่งไคลเอนต์

ข้อความแฟลชสำหรับวัฏจักร POST → redirect → GET ไม่สามารถใช้เซสชันของ Xojo ในโหมด SSR ได้ — เซสชันจะหายไปเมื่อเวลาที่ GET ของการเปลี่ยนเส้นทางมาถึง วิธีแก้ปัญหาคือ เขียนไปที่ `sessionStorage` ก่อนที่แบบฟอร์มจะส่ง อ่านในการโหลดหน้าต่อไป
...
## รูปแบบ 3 — ส่งแบบฟอร์มพร้อมการประมวลผลแบบ Async

แบบฟอร์มยืนยันตัวตนต้องแฮชรหัสผ่านฝั่งไคลเอนต์ด้วย Web Crypto API ก่อนส่ง POST สิ่งนี้ทำแบบ synchronous ไม่ได้ จึงต้องดักจับ submit event ไว้ก่อน Alpine's `@submit.prevent` + async method ใน `x-data` จัดการได้อย่างเรียบร้อย โดยไม่ต้องใช้ `addEventListener` หรือ script block แยกต่างหาก
