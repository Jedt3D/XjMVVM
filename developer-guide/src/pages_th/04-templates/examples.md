...
ตัวแปร `flash` เป็น `Dictionary` ที่มีกุญแจสองตัว: `type` (`"success"`, `"error"`, `"info"`) และ `message` (สตริงข้อความ) ตัวแปรนี้ถูกฉีดเข้าโดยอัตโนมัติ — ViewModel ใดๆ สามารถเรียก `SetFlash()` และข้อความจะปรากฏบนหน้าที่แสดงผลครั้งต่อไป
...
จุดสำคัญคือ `note.id`, `note.title`, `note.body`, `note.updated_at` เป็นกุญแจของ Dictionary การลบทำผ่าน `POST` form เพราะแบบฟอร์ม HTML รองรับเพียง GET และ POST เท่านั้น — ไม่มี HTTP DELETE ในแบบฟอร์มบราวเซอร์
...
`NotesNewVM` ไม่ส่งกุญแจ `note` (หรือส่ง `Nil`) — เทมเพลตแสดงผลในโหมดสร้าง `NotesEditVM` ส่ง Dictionary `note` ที่โหลดแล้ว — เทมเพลตจะเติมค่าที่มีอยู่แล้วล่วงหน้า
