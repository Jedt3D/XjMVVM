---
title: "ระบบการตรวจสอบอัตลักษณ์"
description: วิธีที่ XjMVVM จัดการการลงทะเบียนผู้ใช้, การเข้าสู่ระบบ, การออกจากระบบ, การแฮชรหัสผ่าน, และการตรวจสอบอัตลักษณ์แบบ cookie สำหรับโหมด SSR
---

# ระบบการตรวจสอบอัตลักษณ์

การตรวจสอบอัตลักษณ์ใน XjMVVM สร้างขึ้นจากสามชั้น: **`UserModel`** จัดเก็บและตรวจสอบข้อมูลประจำตัว, **`BaseViewModel`** มีเมธอด auth ที่ใช้ cookie เพื่อปกป้องเส้นทางและเปิดเผยผู้ใช้ปัจจุบันให้กับเทมเพลต, และ **JavaScript ฝั่งเบราวเซอร์** เชื่อมช่องว่างเซสชัน SSR สำหรับสถานะเนวิเกชันและข้อความแฟลช

!!! warning
    **`Self.Session` มีค่าเป็น Nil เสมอในโหมด SSR** Xojo Web 2 ของ `WebSession` ต้องการการเชื่อมต่อ WebSocket ที่คงอยู่ ในโหมด SSR บริสุทธิ์ — ซึ่ง `HandleURL` จะคืนค่า `True` เสมอ — ไม่มี WebSocket และไม่มีเซสชัน การตรวจสอบอัตลักษณ์ทั้งหมดใช้ cookie ที่ลงนาม HMAC แทน
...
ก่อนฟอร์มการเข้าสู่ระบบหรือการลงทะเบียนถูกส่ง เบราวเซอร์จะแฮชรหัสผ่านโดยใช้ [Web Crypto API](https://developer.mozilla.org/en-US/docs/Web/API/SubtleCrypto/digest) เพื่อไม่ให้รหัสผ่านเป็นข้อความธรรมชาติข้ามเครือข่าย:
...
!!! note
    `crypto.subtle` พร้อมใช้งานในเบราวเซอร์สมัยใหม่ทั้งหมด และไม่ต้องการไลบรารี่ใด ๆ มันพร้อมใช้งานเฉพาะในบริบทที่ปลอดภัย (HTTPS หรือ localhost)
...
## การตรวจสอบอัตลักษณ์โดยใช้ Cookie

เนื่องจาก `Self.Session` มีค่าเป็น Nil เสมอในโหมด SSR, XjMVVM ใช้ cookie HTTP ที่ลงนาม HMAC สำหรับการตรวจสอบอัตลักษณ์ cookie ถูกตั้งค่าผ่านส่วนหัว HTTP `Set-Cookie` และตรวจสอบในทุกคำขอ
...
`App.mAuthSecret` เป็นสตริงเลขฐานสิบหก 32 ไบต์แบบสุ่มที่สร้างครั้งเดียวในตอนเริ่มต้นผ่าน `Crypto.GenerateRandomBytes(32)` มันเปลี่ยนแปลงในทุกการรีสตาร์ท ซึ่งมีผลเอาสิ่งอื่นออกจากการตรวจสอบอัตลักษณ์ cookie ที่มีอยู่ทั้งหมด
...
เรียกในทุกคำขอเพื่อแยกและตรวจสอบผู้ใช้ที่ได้รับการตรวจสอบอัตลักษณ์ ผลลัพธ์ถูกแคชต่อคำขอผ่านคุณสมบัติ `mAuthParsed` / `mAuthCache` เพื่อหลายการเรียก (จาก `RequireLogin`, `CurrentUserID`, `Render`) ไม่ต้องแยกวิเคราะห์ใหม่
...
หลังจากการตรวจสอบอัตลักษณ์สำเร็จ `RedirectWithAuth` ทำสามสิ่งในการตอบสนองเดียว:
...
3. เปลี่ยนเส้นทางเบราวเซอร์ไปยัง URL เป้าหมาย
...
- `Set-Cookie` ตั้งค่า cookie HTTP (สำหรับการตรวจสอบอัตลักษณ์ฝั่งเซิร์ฟเวอร์ในคำขอต่อมา)
...
`Max-Age=0` บอกเบราวเซอร์ให้ลบ cookie ทันที

---

## ตัวช่วย BaseViewModel Auth

สี่เมธอดบน `BaseViewModel` ทำให้การตรวจสอบอัตลักษณ์เข้าถึงได้จาก ViewModel ใด ๆ โดยไม่จัดการ cookie โดยตรง:
...
ส่งกลับ `0` เมื่อไม่ได้รับการตรวจสอบอัตลักษณ์ — ศูนย์ไม่เคยเป็นคีย์หลักฐานอุปสรรค `AUTOINCREMENT` ของ SQLite ที่ถูกต้อง
...
ปกป้องเส้นทาง HTML เปลี่ยนเส้นทางผู้ใช้ที่ไม่ได้รับการตรวจสอบอัตลักษณ์ไปยังหน้าเข้าสู่ระบบพร้อมพารามิเตอร์ `next` เพื่อให้พวกเขากลับไป URL ดั้งเดิมหลังจากเข้าสู่ระบบ
...
|-----------|---------|---------|----------|
| cookie `mvvm_auth` | cookie HTTP | การตรวจสอบอัตลักษณ์ฝั่งเซิร์ฟเวอร์ | จนกว่าเบราวเซอร์จะล้างหรือ `Max-Age=0` |
| `localStorage._auth_user` | localStorage เบราวเซอร์ | การแสดงเนวิเกชันฝั่งไคลเอนต์ | คงอยู่ในแท็บ/การรีสตาร์ท |
| `sessionStorage._flash_msg` | sessionStorage เบราวเซอร์ | ข้อความแฟลชแบบครั้งเดียว | แท็บปัจจุบันเท่านั้น อยู่รอดได้จากการเปลี่ยนเส้นทาง |
...
### สถานะการตรวจสอบอัตลักษณ์เนวิเกชัน — `localStorage`
